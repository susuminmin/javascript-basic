...
def like(request, article_pk):
    ...
    if user in article.liked_users.all():
        # if article.liked_users.filter(pk=user.pk).exists(): filter는 get으로 가져올때 에러내는것 방지..?, exists(): 데이터 하나라도 존재하면 True 반환
        user.liked_articles.remove(article)
        liked = False
    else:
        user.liked_articles.add(article)  # 유저가 좋아요 한 아티클을 가져와서 추가하겠다.
        liked = True
